                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-12 text-right mb-3">
                            <a href="{{ route('flights.index', $flight) }}" class="btn btn-primary">@lang("Back")</a>
                        </div>
                    </div>
                    @if ($errors->any())
                    ...
                    <form method="post" action="{{ route('flights.update', $flight)}}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="form-group">
                            <label><strong> {{__("Name")}} :</strong></label>
                            <input type="text" name="name" class="form-control"  value="{{old('name',$flight->name)  }}"/>
                            {!! $errors->first('size','<small>:message</small><br>') !!}
                        </div>

                        <div class="form-group">
                            <label><strong>@lang("Date"):</strong></label><br>
                            <input type="date" name="date" class="form-control" value="{{old('date',$flight->date)}}"/>
                            {!! $errors->first('size','<small>:message</small><br>') !!}
                        </div>

                        <div class="form-group">
                            <label><strong>@lang("Number of hours done"):</strong></label><br>
                            <input type="number" name="number" class="form-control" value="{{old('number',$flight->number)}}"/>
                            {!! $errors->first('size','<small>:message</small><br>') !!}
                        </div>
