                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-12 text-right mb-3">
                            <a href="{{ route('agenda.index', $agenda) }}" class="btn btn-primary">@lang("Back")</a>
                        </div>
                    </div>
                    @if ($errors->any())
                    ...
                    <form method="post" action="{{ route('agenda.update', $agenda)}}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="form-group">
                            <label><strong> {{__("Name")}} :</strong></label>
                            <input type="text" name="name" class="form-control"  value="{{old('name',$agenda->name)  }}"/>
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="number" class="form-control" value="{{old('number',$agenda->number)}}"/>
                            {!! $errors->first('size','<small>:message</small><br>') !!}
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" rows="4" cols="40" name="description">{{old('description',$agenda->description)}}</textarea>
                        </div>

                        <div class="form-group">
                            <label><strong>@lang('Email') :</strong></label>
                            <input type="email" name="email" class="form-control" value="{{old('email',$agenda->email)}}"/>
                        </div>
